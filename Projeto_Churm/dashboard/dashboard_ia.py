# dash.py - Versão usando requests (igual ao seu outro dashboard)
...
def gerar_insights_deepseek(df_filtrado, resultados):
    """Gera insights usando a API da DeepSeek"""
...
**🤖 DeepSeek API**
- Modelo: DeepSeek-V3
- Status: {'✅ Conectado' if cliente.api_key else '❌ Não conectado'}
""")
...
"""
                ...
                resposta = cliente.chat(messages, temperature=0.5, max_tokens=1000)
...
"""
    ...
    st.download_button(
        ...
        data=relatorio,
        file_name=f"relatorio_churn_{datetime.now().strftime('%Y%m%d_%H%M')}.txt",
        mime="text/plain"
    )
